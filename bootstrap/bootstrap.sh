...
if [ ! -d "openshift-gitops-operator/overlays/${OVERLAY}" ]; then
    echo "Overlay ${OVERLAY} does not exist in path openshift-gitops-operator/overlays/${OVERLAY}"
    exit 1
fi
...
kustomize build "openshift-gitops-operator/overlays/${OVERLAY}" | oc apply -f -
...
kustomize build openshift-gitops-instance/overlays/${OVERLAY} | oc apply -f -
...
echo "Apply cluster config application"
kustomize build cluster-config/ | oc apply -f -
